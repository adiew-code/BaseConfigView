...
#import "MJDictDBService.h"
#import "MJDictObject.h"
#import "MJDistributorDBService.h"
#import "MJDistributorObject.h"
#import "MJCityInfoDBService.h"
#import "MJCityInfoObject.h"
#import "MJUserDBService.h"
...
-(instancetype)initWithConfigParam:(NSDictionary *)configParam
{
    ...
    NSString *dataSourceQueryParam = configParam[kWidgetParam_datasourceIndex];
    if (dataSourceQueryParam.length)
    {
        NSArray * array = [self getSingleDropDatasource:dataSourceQueryParam];
        if (array.count)
        {
...
-(NSArray *)getSingleDropDatasource:(NSString *)VF_C{
   
    NSArray * Dicts = nil;
    if ([VF_C isEqualToString:@"OutletType_2__c"] || [VF_C isEqualToString:@"Outlet_Change_Reason__c"] || [VF_C isEqualToString:@"ChannelGroup__c"] || [VF_C isEqualToString:@"PromotionActivities__c"] || [VF_C isEqualToString:@"NationalPromotion__c"] || [VF_C isEqualToString:@"KAPromotion__c"] || [VF_C isEqualToString:@"otherPromotion__c"]) {
         MJDictDBService * dictDB = [[MJDictDBService alloc]init];
        Dicts = [dictDB queryDictObjectsWithF__C:VF_C];
    }else if ([VF_C isEqualToString:@"Outlet_Type__c"]){
         MJDictDBService * dictDB = [[MJDictDBService alloc]init];
        Dicts = [dictDB queryDictObjectsWithVF__C:VF_C];
    }else if ([VF_C isEqualToString:@"DistributorInfo"]){
        MJDistributorDBService * dictDB = [[MJDistributorDBService alloc]init];
        Dicts = [dictDB queryAllObjects:[MJDistributorObject class]];
    }else if ([VF_C isEqualToString:@"CityList"]){
        MJCityInfoDBService * dictDB = [[MJCityInfoDBService alloc]init];
        Dicts = [dictDB queryAllObjects:[MJCityInfoObject class]];
    }else if ([VF_C isEqualToString:@"UserInfo"]){
        MJUserDBService * userDBService = [[MJUserDBService alloc]init];
        Dicts = [userDBService queryAllObjects:[MJUserObject class]];
    }
    else{
         MJDictDBService * dictDB = [[MJDictDBService alloc]init];
        Dicts = [dictDB queryObjectsWithClass:[MJDictObject class] fieldName:@"lable__c" fieldValue:VF_C];
    }
    
//    NSMutableArray *DictsDatasource = [[NSMutableArray alloc]init];
//    for (int i = 0; i < Dicts.count; i++) {
//        MJDBObjectBase *object = Dicts[i];
//        MJSingleDropViewModel  *model = [[MJSingleDropViewModel alloc]init];
//        if ([object isKindOfClass:[MJDictObject class]]) {
//            MJDictObject * dictObject = (MJDictObject *)object;
//            model.index = [NSString stringWithFormat:@"%d",i];
//            model.dropCont = dictObject.value__c;
//
//        }else if ([object isKindOfClass:[MJDistributorObject class]]){
//            MJDistributorObject * dictObject = (MJDistributorObject *)object;
//            model.index = [NSString stringWithFormat:@"%d",i];
//            model.dropCont = dictObject.Name;
//        }else if ([object isKindOfClass:[MJCityInfoObject class]]){
//            MJCityInfoObject * dictObject = (MJCityInfoObject *)object;
//            model.index = [NSString stringWithFormat:@"%d",i];
//            model.dropCont = dictObject.Name;
//        }
//        [DictsDatasource addObject:model];
//    }
    return Dicts;
}
